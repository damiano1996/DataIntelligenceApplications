...
def step3(m):
    dim = m.shape[0]
    assigned = np.array([])
    assignements = np.zeros(m.shape, dtype=int)
    ...
    for i in range(0, dim):
        for j in range(0, dim):
            ...
            if (
                    m[i, j] == 0 and
                    np.sum(assignements[:, j]) == 0 and
                    np.sum(assignements[i, :]) == 0
            ):
                assignements[i, j] = 1
    ...
    while len(new_marked_rows) > 0:
        ...
        for nc in new_marked_cols:
            new_marked_rows = np.append(new_marked_rows, np.argwhere(assignements[:, nc] == 1).reshape(-1))
